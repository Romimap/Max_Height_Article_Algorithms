...
#Computing the cdf of the families distribution based on weight
f_prob = [1, 2, 14, 2, 1, 1, 3, 1, 15, 1, 2, 4, 1, 0.1, 0.1, 8]
s = sum(f_prob)
for i in range(16): #distribution
    f_prob[i] /= s
    print('$\\cp F_{' + str(i) + '}$ & ' + "{:.2f}".format(f_prob[i]) + '\\\\\hline')
...
print(
"\\hline\n\
Family & exemplar & synthesis\\\\\n\
\\hline\n")
for i in range(16):
    print(
f"$\\cp F_{'{'+str(i)+'}'}$ & {len(families[i])} & {len(s_families[i])}\\\\\n\
\\hline")
